...
- [x] Decide how the app signs in — both: an access token you make yourself needs nothing registered anywhere, and the device flow is there too, against the existing IdeaBRD OAuth app with Device Flow enabled and its client id set as `IDEABRD_GITHUB_CLIENT_ID`
