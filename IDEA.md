...
- [ ] Decide whether the app gets a registered OAuth client id at all — signing in with your own access token needs nothing registered, so a client id (device flow, `IDEABRD_GITHUB_CLIENT_ID`) is now a convenience rather than a requirement, and whoever registers it has their name on everyone's consent screen
